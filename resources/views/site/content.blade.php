...
<section id="{{ $page->alias }}" class="top_cont_outer">
  <div class="hero_wrapper">
    <div class="container">
      ...
      {!! Html::image('assets/img/'.$page->img, '', ['calss'=>'zoomIn wow animated']) !!}
      </div>
        </div>
      </div>
    </div>
...
<div class="inner_wrapper">
  ...
  <div class="row">
      <div class=" col-lg-4 col-md-4 col-sm-4 col-xs-12 pull-right">
        ...
        {!! Html::image('assets/img/'.$page->img, '', ['class'=>'img-circle delay-03s animated wow zoomIn']) !!}
      </div>
...
<section  id="service">
  ...
  @foreach ($services as $service)
        ...
        @endif
          ...
          <div class="col-lg-4{{ $loop->index % 3 !== 0 ? " borderLeft" :'' }}{{ $loop->index >= 3 ? ' mrgTop' : '' }}">
  ...
  <div class="portfolio"> 
    ...
    <div class="isotope fadeInLeft animated wow" style="position: relative; overflow: hidden; height: 480px;" id="portfolio_wrapper"> 
      @foreach ($portfolios as $item)
              <!-- Portfolio Item -->
        <div style="position: absolute; left: 0px; top: 0px; transform: translate3d({{ ($loop->index < 4 ) ? $loop->index*337 : ($loop->index - 3)*337 }}px, {{ $loop->index > 3 ? 240 : 0 }}px, 0px) scale3d(1, 1, 1); width: 337px; opacity: 1;" class="portfolio-item one-four   {{ $item->filter }} isotope-item">
  ...
  <div class="container">
    ...
    <div class="team_section clearfix">
      @foreach ($peoples as $people)
        <div class="team_area">
          <div class="team_box wow fadeInDown delay-0{{ $loop->index*3 }}s">
            <div class="team_box_shadow"><a href="javascript:void(0)"></a></div>
            {!! Html::image('assets/img/'.$people->img, $people->name) !!}
          </div>
          <h3 class="wow fadeInDown delay-0{{ $loop->index*3 }}s">{{ $people->name }}</h3>
          <span class="wow fadeInDown delay-0{{ $loop->index*3 }}s">{{ $people->position }}</span>
          <p class="wow fadeInDown delay-0{{ $loop->index*3 }}s">{{ $people->text }}</p>
        </div>
      @endforeach
    {{--  --}}
    </div>
        ...
        <div class="col-lg-8 wow fadeInLeft delay-06s">
          <div class="form">
            <form action="{{ route('home') }}" method="POST">
              ...
              <textarea name="text" class="input-text text-area" cols="0" rows="0" onFocus="if(this.value==this.defaultValue)this.value='';" onBlur="if(this.value=='')this.value=this.defaultValue;" placeholder="Your Message *"></textarea>
